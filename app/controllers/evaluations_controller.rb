class EvaluationsController < ApplicationController
  ...
  def export_evals
    ...
    roster_csv = CSV.generate do |csv|
      ...
      csv << ["Student", "Reviewer", "Effort", "Reliability","Quality", "Comments"]
      ...
      myproject.evaluations.each do |eval|
        eval.eval_relations.each do |relation|
          ...
          csv << [user.first_name + " " + user.last_name, reviewer.first_name + " " + reviewer. last_name, eval.effort, eval.reliability, eval.quality, eval.comments]
        end 
      end
    end
