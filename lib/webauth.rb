# WebAuth module to be included into ActionController
...
module WebAuth
  ...
  LOGGER=Logger.new(File.join(RAILS_ROOT,'log','webauth.log'))
  ...
  BASE_URI = "https://#{SERVER}/ucinetid"
  LOGIN_URI    = "#{BASE_URI}/webauth"
  LOGOUT_URI   = "#{BASE_URI}/webauth_logout"
  CHECK_URI   = "#{BASE_URI}/webauth_check"
  ...
  def set_webauth_session(session_var,webauth_field)
    session[:webauth]={:auth_key => @check.auth_key, :expires => Time.now+RECHECK_TIME}
  ...
  class Check 
    ...
    def parse_and_load_results(check_results)
      return nil unless check_results
      results=XmlSimple.xml_in(check_results,{'ForceArray' => false})
